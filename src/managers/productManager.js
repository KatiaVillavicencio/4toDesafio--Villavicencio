   ...
   addProduct = async (obj) => {
    ...
    if (!title || !description || !price || !category || !code ||!status || !stock) {
      ...
      if (codigorepetido) {
        ...
        const productnew = {
          id,title,description,price,category,status,thumbnail,code, stock,
        };
 ...
 //Generate id
  ...
  updateProduct = async (id,obj) => {
    ...
    const {title, description, price, category,thumbnail, status,code, stock}=obj
      ...
      if (codigorepetido) {
        console.error(
          `Código para actualizar ${code} repetido`);
        return;
      } else {
        const listadoProductos = await this.getProducts({});
        const newProductsList = listadoProductos.map((elemento) => {
          if (elemento.id === parseInt(pid)) {
                    const updatedProduct = {
                      ...elemento,title,description,price,category,status,thumbnail,code,stock
                    };
